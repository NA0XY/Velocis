...
import defaultShape from './MetallicPaint.svg';
...
export default function MetallicPaint({
  ...
  useEffect(() => {
    ...
    const render = time => {
      ...
      // Accumulate raw elapsed time in seconds
      animTimeRef.current += (delta / 1000) * speedRef.current;
      ...
      if (mouseAnimRef.current) {
        ...
        shaderTime = mouse.x * 3000 + mouse.y * 1500;
      } else {
        // Primary sine wave: smooth ease-in/ease-out ping-pong (~12s cycle at speed 0.4)
        const primary   = Math.sin(t * 0.52) * 0.5 + 0.5;
        // Slower secondary wave: subtle drift so it never feels perfectly looped
        const secondary = Math.sin(t * 0.17 + 1.2) * 0.5 + 0.5;
        // 80% primary sweep + 20% slow drift, mapped to shader range [0, 3000]
        shaderTime = (primary * 0.8 + secondary * 0.2) * 3000;
      }

      gl.uniform1f(u.u_time, shaderTime);
